(() => {
    ...
    function initSliders() {
        if (document.querySelector(".talents__slider")) new swiper_core_Swiper(".talents__slider", {
    ...
    const subLang = document.querySelector(".header__sublang");
    headerLang.addEventListener("click", (function() {
        subLang.classList.toggle("var_active");
        headerLang.classList.toggle("lang_active");
    }));
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        const observer = new IntersectionObserver(((entries, observer) => {
            entries.forEach((entry => {
                const targetElement = entry.target;
                if (entry.isIntersecting && targetElement.querySelectorAll("[data-digits-counter]").length) {
        ...
        const aboutNums = document.querySelectorAll(".about__num");
        aboutNums.forEach((item => observer.observe(item)));
    }));
    const subscriberInput = document.getElementById("subscribers");
    const subscriberValue = document.getElementById("subscriberValue");
    const monthEarnings = document.querySelector(".calc__month");
    const quarterEarnings = document.querySelector(".calc__cvartal");
    const yearEarnings = document.querySelector(".calc__year");
    function calculateEarnings(subscribers) {
        const earningsPer10k = 1e3;
        const earnings = subscribers / 1e4 * earningsPer10k;
        return earnings;
    }
    function updateEarnings() {
        const subscribers = subscriberInput.value;
        const earnings = calculateEarnings(subscribers);
        subscriberValue.textContent = formatNumber(subscribers);
        monthEarnings.textContent = `$${formatNumber(earnings)}/месяц`;
        quarterEarnings.textContent = `$${formatNumber(earnings * 3)}/3 месяца`;
        yearEarnings.textContent = `$${formatNumber(earnings * 12)}/год`;
    }
    function formatNumber(number) {
        return number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    }
    updateEarnings();
    subscriberInput.addEventListener("input", updateEarnings);
    window["FLS"] = true;
    menuInit();
    spollers();
    formFieldsInit({
